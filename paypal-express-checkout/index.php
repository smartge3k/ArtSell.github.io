...
if(isset($_GET["token"]) && isset($_GET["PayerID"]))
{
    ...
    {		
      ...
      $ItemTotalPrices = $ItemTotalPrice;
        $ItemName =  urlencode($p_item['itm_name']);
        $ItemNumber = urlencode($p_item['itm_code']);
        
        $ItemQTY = urlencode($p_item['itm_qty']);
    }
			...
			$transactionID = urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
				...
				*/
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
				{
					...
					(BuyerName, BuyerEmail, TransactionID, ItemName, ItemNumber, ItemAmount, ItemQTY, UserId)
					VALUES ('$buyerName','$buyerEmail','$transactionID','$ItemName', '$ItemNumber', '$ItemTotalPrices', '$ItemQTY', '$userid')");
					...
					echo '<h3 style="text-align:center"> Your Transaction has been success full View Your Dashboard -> Order Item section to claim your Product or Click link bellow</h3>';
					echo '<a href="../userindex.php"> Click Here!</a>';
